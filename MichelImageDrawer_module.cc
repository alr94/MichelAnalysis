...
class MichelAnalysis::MichelImageDrawer : public art::EDAnalyzer 
{

public:

	explicit MichelImageDrawer(const Parameters & config);

	// Plugins should not be copied or assigned.
	MichelImageDrawer(MichelImageDrawer const &) = delete;
	MichelImageDrawer(MichelImageDrawer &&) = delete;
	MichelImageDrawer & operator = (MichelImageDrawer const &) = delete;
	MichelImageDrawer & operator = (MichelImageDrawer &&) = delete;
...
//       dataprovider
MichelAnalysis::MichelImageDrawer::MichelImageDrawer(Parameters const & config) 
...
void MichelAnalysis::MichelImageDrawer::beginJob()
{
	...
	MichelAnalysis::MichelImageDrawer::InitialiseHitTree(tfs);
	MichelAnalysis::MichelImageDrawer::InitialiseDaughterTree(tfs);
	MichelAnalysis::MichelImageDrawer::InitialiseEventSelectionTree(tfs);

}

void MichelAnalysis::MichelImageDrawer::endJob()
{
...
void MichelAnalysis::MichelImageDrawer::InitialiseHitTree(
  art::ServiceHandle<art::TFileService> tfs)
{
...
void MichelAnalysis::MichelImageDrawer::InitialiseDaughterTree(
  art::ServiceHandle<art::TFileService> tfs)
{
...
void MichelAnalysis::MichelImageDrawer::InitialiseEventSelectionTree(
  art::ServiceHandle<art::TFileService> tfs)
{
...
void MichelAnalysis::MichelImageDrawer::analyze(art::Event const & event)
{
	...
	// Get simulated objects
	std::cout << "-------------------------------- Getting Simulated Objects "
	          << "--------------------------------" <<std::endl;
	...
	// Find the number of true Michel electrons expected in this event
	fEventSelection.NTrue += MichelAnalysis::CountDecayingMuons(*mcparticles, 
	                                                            particleMap,
	                                                            duneUtils, 
	                                                            fFidVolCut); 
	std::cout << "True michels in event: " << fEventSelection.NTrue << std::endl;
	...
	for (auto const & pfparticle : *pfparticles) 
	{
		...
		bool primarySelected { false }, correctlySelected { false };
		...
		trackData.IsMuonDecaying = 
		  MichelAnalysis::TrackIsDecayingMuon(primarytrack, event, pdUtils,
		                                      fTrackTag, particleMap);
		...
		for (auto const & daughterShower : daughterShowers) 
		{
			...
			// Purity and efficiency tests
			MichelAnalysis::EventSelectionTests(fEventSelection, trackData, pdUtils, 
			                                    primarytrack, particleMap, event, 
			                                    fTrackTag);
			correctlySelected = trackData.IsMuonDecaying;
			...
			{
				fCanvas -> SaveAs(Form("good_patch_%i.png", nImagesDrawn));
				fCanvas -> Clear();
			}
			else
			{
				fCanvas -> SaveAs(Form("bad_patch_%i.png", nImagesDrawn));
				fCanvas -> Clear();
			}
...
DEFINE_ART_MODULE(MichelAnalysis::MichelImageDrawer)
